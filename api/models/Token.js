...
class Token {
    ...
    static async create(user_id) {
        ...
        const newId = response.row[0].token_id;
    ...
    async deleteToken() {
        const response = await db.query('DELETE FROM token WHERE token = $1 RETURNING token', [this.token]);
        if (response.rows.length != 1) {
            throw new Error("Cannot Delete a Token that does not Exist");
        }
        else {
